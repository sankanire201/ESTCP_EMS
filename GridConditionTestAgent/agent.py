...
def gridConditionTestAgent(config_path, **kwargs):
    ...
    :returns: Gridconditiontestagent
    :rtype: Gridconditiontestagent
    """
    try:
    ...
    return Gridconditiontestagent(setting1,
                          setting2,
                          **kwargs)
def listen(client, userdata, message):
    global message1
    message1=message
    if message.topic=="ems/control/gridalart":
	 Gridconditiontestagent.Grid_alart=message.payload
...
class Gridconditiontestagent(Agent):
    Grid_alart=''
    """
    ...
    def __init__(self, setting1=1, setting2="some/random/topic",
                 **kwargs):
        super(Gridconditiontestagent, self).__init__(**kwargs)
        ...
        self.GC_client= paho.Client("GC_client")
        self.GC_client.on_connect=connect
        self.GC_client.on_message=listen
        self.GC_client.connect("192.168.10.52",1883)
        self.GC_client.subscribe("ems/control/gridalart/#")
        self.GC_client.publish("ems/control/gridalart",'none')
        self.GC_client.loop_start()
        ...
        csv_path='/home/sanka/CENTRAL_CONTROL_UNIT/WeMo_Config2.csv'
        WeMo_Priorities={}
	config_dict = utils.load_config('/home/sanka/CENTRAL_CONTROL_UNIT/WeMo_Config2.csv')
	self.loads_consumption={}
        self.total_consumption=0
        self.critical={}
        self.essential={}
    ...
    def onstart(self, sender, **kwargs):
        ...
        #Exmaple RPC call
#zddddddddddddddddddddddddddddaygrrrrrrrrruuuuuuuuus````222ssss        r2q22312zzaq1f[]]9w xcvbgevvvvvvvvvvvqq qqqqqqq q q tcq cqcqsa1aa`        qq        #self.vip.rpc.call("some_agent", "some_method", arg1, arg2)
    ...
    # }
       ...
       # Message={"Total_WeMo_Consumption": {"Readings": [utils.format_timestamp(utcnow),total],"Units": "Kw","tz": "UTC","data_type": "int"}}
        ...
        print("##########################################################################toooo much",Gridconditiontestagent.Grid_alart,self.threshhold,self.total_consumption,self.criticaltotalconsumption,self.essentialtotalconsumption,self.nonessentialtotalconsumption)
#self.vip.pubsub.publish('pubsub',"control/plc/shedding",message=int(total_consumption-self.threshhold))
#self.total_consumption,self.criticaltotalconsumption,self.essentialtotalconsumption,self.nonessentialtotalconsumption
        if Gridconditiontestagent.Grid_alart=='conserve':
            self.vip.pubsub.publish('pubsub',"control/plc/shedding",message=int(0.3*self.nonessentialtotalconsumption))
            Gridconditiontestagent.Grid_alart='none'
            print("##########################################################################Conserve",int(0.3*self.nonessentialtotalconsumption))
        if Gridconditiontestagent.Grid_alart=='powerwatch':
            self.vip.pubsub.publish('pubsub',"control/plc/shedding",message=int(self.nonessentialtotalconsumption+0.5*self.essentialtotalconsumption))
            Gridconditiontestagent.Grid_alart='none'
            print("##########################################################################Powerwatch",int(self.nonessentialtotalconsumption+0.5*self.essentialtotalconsumption))

        if Gridconditiontestagent.Grid_alart=='powerwarning':
            self.vip.pubsub.publish('pubsub',"control/plc/shedding",message=int(self.nonessentialtotalconsumption+self.essentialtotalconsumption))
            Gridconditiontestagent.Grid_alart='none'
            print("##########################################################################Powerwarning",int(self.nonessentialtotalconsumption+self.essentialtotalconsumption))

        if Gridconditiontestagent.Grid_alart=='emergency':
            self.vip.pubsub.publish('pubsub',"control/plc/shedding",message=int(self.total_consumption))
            Gridconditiontestagent.Grid_alart='none'
            print("##########################################################################Emergency",int(self.total_consumption))
...
def main():
    ...
    utils.vip_main(gridConditionTestAgent, 
                   version=__version__)
